...
%% Link-5_6
trf_viz_link1p68= make_transform([0.1, 0, 0], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(3)= link_box([0.2, 0.02, 0.04], trf_viz_link1p2, [0, 0, 1]); 
plot_axes(trf_viz_link1p2, ' ', true, axis_length); % V_{1-2}

%% Link-5_7
trf_viz_link1p38= make_transform([0.2, 0, 0], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(4)= link_cylinder(0.035, 0.05, trf_viz_link1p3, [0, 0, 1]); 
% plot_axes(trf_viz_link1p3, ' ', true, axis_length); % V_{1-3}

%% Link-6
trf_viz_link28= make_transform([0, 0, 0], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(5)= link_cylinder(0.03, 0.04, trf_viz_link2, [1, 0.54902, 0]); 
% V_2 and L_2 are the same.

%% Link 6-6
trf_viz_link2p58= make_transform([0, -0.05, 0], pi/2, 0, 0); % Do not specify parent yet: It will be done in the joint
h(6)= link_cylinder(0.015, 0.1, trf_viz_link2p2, [1, 0.54902, 0]); 
% plot_axes(trf_viz_link2p2, ' ', true, axis_length); % V_{2-2}

%% Link-7
trf_viz_link98= make_transform([0, 0, -0.03], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(7)= link_cylinder(0.012, 0.06, trf_viz_link3, [0.196078, 0.803922, 0.196078]); 
% plot_axes(trf_viz_link3, ' ', true, axis_length); % V_3

%% Link-7-6
trf_viz_link3p78= make_transform([0, 0, 0], pi/2, 0, 0); % Do not specify parent yet: It will be done in the joint
h(8)= link_cylinder(0.012, 0.04, trf_viz_link3p2, [0.196078, 0.803922, 0.196078]); 
% plot_axes(trf_viz_link3p2, ' ', true, axis_length); % V_{3-2}

%% Link-8
trf_viz_link78= make_transform([0, 0, -0.075], 0, 0, 0); % Do not specify parent yet: It will be done in the joint
h(9)= link_cylinder(0.004, 0.15, trf_viz_link4, [0.482353 0.407843 0.933333]); 
% plot_axes(trf_viz_link4, ' ', true, axis_length); % V_{4}
